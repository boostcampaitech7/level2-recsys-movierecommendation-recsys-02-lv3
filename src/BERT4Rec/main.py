...
def main(config):
    ...
    user_train, user_valid, num_user, num_item = preprocess_data(data_path, config.dataloader.batch_size)
    ...
    # 학습
    train(model, dataloader, criterion, optimizer, device, config.train.epochs)
    ...
    model_path = os.path.join(config.train.ckpt_dir, "BERT4Rec.pth")
    ...
    if config.predict:
        ...
        bert4rec_predict(model, users, user_train, num_user, num_item, config.model_args.BERT4Rec.max_len, idx2user, idx2item, output_path, predict_save_path, device)
