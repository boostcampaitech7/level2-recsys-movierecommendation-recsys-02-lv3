...
class BERT4Rec(nn.Module):
    def __init__(self, num_user, num_item, num_genres, hidden_units, num_heads, num_layers, max_len, dropout_rate, device):
        ...
        self.genre_emb = torch.nn.Embedding(num_genres + 1, hidden_units)
    ...
    # 출력 : (batch_size, max_lens, n_items + 1)
    # 총 입력 시퀀스가 max_lens라고 할 때 각 시퀀스에 n_items + 1 개의 확률이 다 나와있다고 생각.
    # n_items + 1에서 1은 mask에 해당하는 차원
    def forward(self, log_seqs, log_genres):
        log_genres_embed = torch.mean(self.genre_emb(torch.LongTensor(log_genres).to(self.device)), dim = -2)
        seqs = self.item_emb(torch.LongTensor(log_seqs).to(self.device))
        seqs = seqs + log_genres_embed
