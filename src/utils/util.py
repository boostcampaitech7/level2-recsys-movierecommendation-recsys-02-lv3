...
class Setting:
    ...
    def model_modular(self, args, filename = 'dataset', fun_name = None):
        ...
        # src/BERT4Rec_with_side_info
        model_folder = os.path.join(args.dataloader.data_path, model_name)
        # src/BERT4Rec_with_side_info/BERT4Rec_with_side_info
    ...
    def save_file(self, args, data, file_extension=".npy", type = None):
        ...
        filename = os.path.join(args.train.predict_dir, f'{self.save_time}_{args.model}')

        # 데이터 타입에 따라 처리
        if file_extension == '.npy':
            filename = os.path.join(args.train.predict_dir, f'{self.save_time}_{args.model}{file_extension}')
            if isinstance(data, np.ndarray):
                np.save(filename, data)
            elif isinstance(data, torch.Tensor):
                np.save(filename, data.cpu().numpy()) 
            else:
                raise ValueError("지원하지 않는 데이터 형식입니다. NumPy 배열 또는 PyTorch 텐서만 허용됩니다.")
        elif file_extension == '.pkl':
            filename = os.path.join(args.train.predict_dir, f'{self.save_time}_{args.model}_' + type + f'{file_extension}')
    ...
    def get_latest_file(self, directory, model, file_extension=".npy"):
        ...
        if file_extension == ".pkl":
            ...
            if user_files:
                latest_user_file = max(user_files, key=lambda x: x.split('_')[0] + x.split('_')[1])
            else:
                latest_user_file = None
            
            if item_files:
                latest_item_file = max(item_files, key=lambda x: x.split('_')[0] + x.split('_')[1])
            else:
                latest_item_file = None

            # 파일 경로 반환 (없으면 None 반환)
            latest_user_path = os.path.join(file_path, latest_user_file) if latest_user_file else None
            latest_item_path = os.path.join(file_path, latest_item_file) if latest_item_file else None
